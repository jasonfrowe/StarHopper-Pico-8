...
- Music: one cart per tune in `music/`, loaded with `music_play(name)` (`src/music.lua`). Sfx 0-51 belong to music, 52-62 to game sound effects (`tools/make_sfx.py`, played via `snd(s_name)` in `src/sound.lua`, on channel 3).
